...
if test "${3:-}" = ""; then
    ...
    mkdir -p ~/tmp
    cd ~/tmp || exit 1
    git clone -b experimental2 "https://github.com/${ORG}/resources.git"
fi
...
if test $INTEGRATION_LEVEL -ge 4; then
    if test $DEBUG -eq 1; then
        _force.sh "${REPO}" experimental2 "${INTEGRATION_LEVEL}" DEBUG ${RESOURCES}
    else
        _force.sh "${REPO}" experimental2 "${INTEGRATION_LEVEL}"
    fi
else
    if test ${DEBUG} -eq 1; then
       _force.sh "${REPO}" experimental2 "${INTEGRATION_LEVEL}" DEBUG ${RESOURCES}
